...
impl WorktreeScriptService {
    pub fn new(events: EventSink, app_data_dir: PathBuf) -> Self {
        Self {
            events: Some(events),
    ...
    fn emit_failure(
        ...
        emit_failure(self.events.as_ref(), request, log_path, exit_code, message);
    }
...
fn emit_failure(
    ...
    if let Some(events) = events {
        events.emit(WORKTREE_SCRIPT_FAILURE_EVENT_NAME, payload);
    }
    warn!("{message}");
}
